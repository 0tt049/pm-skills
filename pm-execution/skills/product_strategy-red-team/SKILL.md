---
name: product_strategy-red-team
...
7. **Optional cross-model mode.** If the user asks for a second opinion and another model (Codex, Gemini, a second Claude) is reachable, use the `invoke_subagent` tool to run the same plan through a subagent and flag where the two disagree — different model families miss different things. Default is single-model; don't add this friction unless asked.
